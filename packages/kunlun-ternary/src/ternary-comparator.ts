...
export class TernaryComparator {
  ...
  static equal<T>(a: T, b: T, tolerance?: number): Trit {
    if (typeof a === 'number' && typeof b === 'number' && tolerance !== undefined) {
      ...
      if (diff <= tolerance * 0.1) return T_TRUE;  // 极小差异 → 确认相等
      if (diff <= tolerance) return T_UNKNOWN;       // 在容差内 → 不确定
   ...
   */
  static semanticMatch(
    similarity: number,
    highThreshold = 0.85,
    lowThreshold = 0.5
  ): Trit {
    if (similarity >= highThreshold) return T_TRUE;
    if (similarity <= lowThreshold) return T_FALSE;
    return T_UNKNOWN;
  }
   ...
   */
  static threshold(value: number, target: number, margin = 0.1): Trit {
    if (target === 0) {
      // 目标为 0 时用绝对值边际
      const absMargin = margin;
      if (value > absMargin) return T_TRUE;
      if (value < -absMargin) return T_FALSE;
      return T_UNKNOWN;
    }
    const ratio = value / target;
    if (ratio > 1 + margin) return T_TRUE;
    if (ratio < 1 - margin) return T_FALSE;
    return T_UNKNOWN;
  }
   ...
   */
  static inRange(value: number, min: number, max: number, margin = 0.05): Trit {
    const range = max - min;
    const buffer = range * margin;
    if (value >= min + buffer && value <= max - buffer) return T_TRUE;
    if (value < min - buffer || value > max + buffer) return T_FALSE;
    return T_UNKNOWN;
  }
}
