...
export class TernaryLogic {
  // ─── 常量 ──────────────────────────────────────
  static readonly TRUE = T_TRUE;
  static readonly UNKNOWN = T_UNKNOWN;
  static readonly FALSE = T_FALSE;
  ...
  static sum(trits: Trit[]): number {
    return trits.reduce((s: number, t) => s + t, 0);
  }
  ...
  static reduceAnd(trits: Trit[]): Trit {
    if (trits.length === 0) return T_TRUE; // 空集 AND = TRUE（幺元）
    return trits.reduce(K3.and);
  }

  /** Trit 数组 OR 归约 */
  static reduceOr(trits: Trit[]): Trit {
    if (trits.length === 0) return T_FALSE; // 空集 OR = FALSE（幺元）
