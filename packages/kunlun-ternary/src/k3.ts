...
const K3 = {
  ...
  and(a: Trit, b: Trit): Trit {
    if (a === -1 || b === -1) return -1; // 任一假 → 假
    if (a === 1 && b === 1) return 1;    // 全真 → 真
    return 0;                             // 含未知 → 未知
  },
  ...
  or(a: Trit, b: Trit): Trit {
    if (a === 1 || b === 1) return 1;   // 任一真 → 真
    if (a === -1 && b === -1) return -1; // 全假 → 假
    return 0;                             // 含未知 → 未知
  },
  ...
  imply(a: Trit, b: Trit): Trit {
    // a=1: 真前提
    if (a === 1) {
      if (b === 1) return 1;   // 真 → 真 = 真
      if (b === 0) return 0;   // 真 → 未知 = 未知（K3 传播）
      return -1;                // 真 → 假 = 假
    }
    // a=0: 未知前提 — 结果不确定（K3 传播）
    if (a === 0) {
      if (b === 1) return 1;   // 未知 → 真 = 真（无论前提如何，结论为真）
  ...
  majority(a: Trit, b: Trit, c: Trit): Trit {
    ...
    if (pos > neg + unk) return 1;   // +1 占绝对多数
    if (neg > pos + unk) return -1;  // -1 占绝对多数
    return 0;                         // 不确定
  },
   ...
   */
  consensus(values: Trit[]): { verdict: Trit; strength: number } {
    const sum = values.reduce((s: number, v) => s + v, 0);
    const strength = values.length > 0 ? Math.abs(sum) / values.length : 0;
    const verdict: Trit = sum > 0 ? 1 : sum < 0 ? -1 : 0;
