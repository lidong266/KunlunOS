...
export async function executeShellWithCapture(
	...
	const appendFullOutput = (text: string): void => {
		...
		writeChain = writeChain.then(async (previous) => {
			...
			if (!appendResult.ok) {
				const appErr = appendResult.error;
				return err(toExecutionError(appErr));
			}
			return ok(undefined);
		});
	};
	...
	const ensureFullOutputFile = (initialContent: string): void => {
		...
		writeChain = writeChain.then(async (previous) => {
			...
			if (!tempFile.ok) {
				const fileErr = tempFile.error;
				return err(toExecutionError(fileErr));
			}
	...
	try {
		...
		if (!result.ok) {
			const execErr = result.error;
			if (execErr.code === "aborted" || options?.abortSignal?.aborted) {
				return ok({
